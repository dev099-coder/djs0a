	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'adicionar') {
				...
				teks = `Ja vai tarde? saiu porque se tivesse entrado não teria saido @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			switch(command) {
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Usernamenya Onde um?', text, {quoted: mek})
        ...
        case 'promover':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Promova o sucesso @${mentioned[0].split('@')[0]} Como um grupo de administração!`, mentioned, true)
				...
				case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Rebaixado com sucesso @${mentioned[0].split('@')[0]} Menjadi Member Group!`, mentioned, true)
